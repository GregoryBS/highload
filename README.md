...
Для каждого датацентра понадобится 1 сервер под Nginx, 4 (по 1 на каждый микросервис) сервера для бэкенда, 3 сервера под БД + ещё 6 для репликации и некоторое кол-во серверов для хранения файлов. На случай отказа какого-либо из серверов должно быть ещё 4 сервера в качестве резервных.
